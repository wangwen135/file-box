(function () {
    ...
    function openChangePasswordModal() {
        ...
        submitBtn.addEventListener('click', () => {
            ...
            if (!current || !next || !confirm) return Notify.warning('请填写所有字段');
            if (next.length < MIN_PWD_LEN) return Notify.warning(`新密码至少 ${MIN_PWD_LEN} 位`);
            if (next !== confirm) return Notify.warning('两次输入的新密码不一致');
            if (next === current) return Notify.warning('新密码不能与当前密码相同');
